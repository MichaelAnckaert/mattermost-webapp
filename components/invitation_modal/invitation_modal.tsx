...
export class InvitationModal extends React.PureComponent<Props, State> {
    ...
    invite = async () => {
        ...
        if (inviteAs === InviteType.MEMBER && this.props.isCloud) {
            trackEvent(TelemetryCategories.CLOUD_INVITE_USERS, 'click_send_invitations', {num_invitations: this.state.invite.usersEmails.length});
        }
        trackEvent(TelemetryCategories.INVITE_USERS, 'click_invite');
